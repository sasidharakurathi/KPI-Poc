...
Copied verbatim from occupancy_prototype/occupancy.py — staff_absence_prototype reuses
OccupancyZone as its inner zone/track-membership engine (see absence.py). The confirmed-inside
count it produces each frame is exactly the "OBJ(person) -> ZONE" input Guard Presence (KPI #19)
needs; absence.py adds the presence predicate, schedule gate, and the empty-duration timer on top.
